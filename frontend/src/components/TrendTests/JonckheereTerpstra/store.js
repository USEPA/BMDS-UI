...
import {getBlob, getHeaders, parseCSVToObjects, toCSV} from "../../../common";
import {exampleDataIndividual, exampleDataSummary} from "./constants";
class Store {
    constructor(token) {
        this.token = token;
        this.selected_dataset = JSON.parse(localStorage.getItem("selected_dataset"));
    ...
    @observable settings = {
        dataset_obj: Object.fromEntries(this.columns.map(col => [col, this.selected_dataset[col]])),
    ...
    loadExampleData() {
        ...
        const parsed_example_data = parseCSVToObjects(this.exampleData, this.columns);
        this.updateSettings(
            "dataset_obj",
            Object.fromEntries(this.columns.map(col => [col, parsed_example_data[col]]))
        );
    }
