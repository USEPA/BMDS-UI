...
const frequentist_model_types = ["frequentist_restricted", "frequentist_unrestricted"];
...
class ModelsStore {
    constructor(rootStore) {
        ...
        reaction(
            () => this.getModelType,
            type => {
                if (type === mc.MODEL_MULTI_TUMOR || type == mc.MODEL_NESTED_DICHOTOMOUS) {
                    this.setActiveTab("mle");
                } else if (type === mc.MODEL_CONTINUOUS && this.activeTab === "toxicr_bayesian") {
    ...
    hasTabs() {
        return (
            this.getModelType === mc.MODEL_CONTINUOUS || this.getModelType === mc.MODEL_DICHOTOMOUS
        );
    }
    ...
    @action.bound setDefaultsForCurrentTab() {
        if (!this.hasTabs()) {
            this.models = models[this.getModelType];
            return;
        } else if (this.activeTab === "loud_bayesian" || this.activeTab === "toxicr_bayesian") {
            this.models[this.activeTab] = models[this.getModelType][this.activeTab] ?? [];
    ...
    @action.bound setModelSelection(name, model, checked) {
        if (checked) {
            ...
            let index = -1;
            if (name === mc.TOXICR_BAYESIAN || name === mc.LOUD_BAYESIAN) {
                index = this.models[name].findIndex(obj => obj._displayName === model);
                if (index > -1) {
                    this.models[name].splice(index, 1);
                    this.setDefaultPriorWeights(name);
                }
    ...
    @action.bound setDefaultPriorWeights(name) {
        const value = parseFloat((this.prior_weight / this.models[name].length).toFixed(4));
    ...
    @action.bound setPriorWeight(type, model, value) {
        let modelIndex = _.findIndex(this.models[type], d => d._displayName === model);
        if (modelIndex >= 0) {
