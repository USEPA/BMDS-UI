...
import {allModelOptions, computeGroupIndex, rowOrder} from "@/constants/modelConstants";
...
const LOUDAccordion = observer(({store, allModels, columns, accordionState}) => {
...
const ModelRow = observer(
    ({store, model, columns, groupIndex = null, getLabel = getDisplayName}) => (
        <tr
            style={{
                backgroundColor: groupIndex != null && groupIndex % 2 !== 0 ? "#f8f9fa" : undefined,
            }}>
            <ModelHeaderTd model={model} writeMode={store.canEdit} getLabel={getLabel} />
            {columns.map(col => {
...
const ModelsCheckBox = observer(({store}) => {
    ...
    let allModels;
    if (store.getModelType === mc.MODEL_CONTINUOUS && activeTab === "loud_bayesian") {
