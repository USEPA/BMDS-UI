...
const getDichotomousData = (dataset, model) => {
    const gof = model.results.gof;
    return {
        headers: ["Dose", "N", "Observed", "Expected", "Estimated Probability", "Scaled Residual"],
...
const getContinuousNormalData = (_dataset, dtype, model) => {
    const gof = model.results.gof;
...
const getContinuousLognormalData = (dataset, dtype, model) => {
    const gof = model.results.gof;
    const useFF = dtype === Dtype.CONTINUOUS_INDIVIDUAL;
    return {
        headers: [
...
class GoodnessFit extends Component {
    render() {
        ...
        const settings = store.modalModel.settings;
        const model = store.modalModel;
        const dataset = store.isMultiTumor ? store.modalDataset : store.selectedDataset;
        const {dtype} = dataset;
        let data;
