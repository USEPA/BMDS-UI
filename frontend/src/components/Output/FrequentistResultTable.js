...
const getModelBinLabel = (output, index) => {
...
const getModelBinText = (output, index) =>
    _.chain(toJS(output.recommender.results.model_notes[index])).values().flattenDeep().value();
const getRecommenderText = (output, index) => {
    const items = getModelBinText(output, index);
    if (items.length === 0) {
        return null;
    }
    return (
...
const getFootnotes = (recommendedModelIndex, selected) => {
    const icons = "*†‡§";
    const fns = [];
...
class RecommendationTd extends Component {
    render() {
        ...
        const popoverTitle = `${name}: ${getModelBinLabel(selectedFrequentist, data.index)}`;
        const popoverContent =
            getModelBinText(selectedFrequentist, data.index).toString().split(",").join("<br/>") ||
            "<i>No notes.</i>";
...
class FrequentistRow extends Component {
    render() {
        ...
        const fns = footnotes.filter(d => d.index === data.index);
        const rowClass = fns.length > 0 ? fns[fns.length - 1].class : "";
        const rowIcon = fns.map(d => d.icon).join("");
...
class FrequentistResultTable extends Component {
    render() {
        ...
        const {selectedFrequentist, isNestedDichotomous} = store;
        ...
        const restrictedModels = getRestricted(models);
        const unrestrictedModels = getUnrestricted(models);
        const colWidths = getColWidths(store);
        const numCols = colWidths.length;
        const recommendedModelIndex = store.recommendationEnabled
            ? selectedFrequentist.recommender.results.recommended_model_index
            : null;
        const footnotes = getFootnotes(recommendedModelIndex, selectedFrequentist.selected);
        const reClass = store.recommendationEnabled ? ` col-l-${numCols}` : "";
