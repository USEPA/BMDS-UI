...
limits = {
    "MCMC": {
        "iterations": {"max_total_iterations": 50_000, "min_per_chain": 2_500},
        "seed": {"min": 0, "max": 2_147_483_647},
        "burnin": {"max_percent": 0.2, "min": 100},
        "chain": {"min": 0, "max": 4},
    },
    "NestedDichotomous": {
        "BootstrapIterations": {"min": 10, "max": 10_000},
        "BootstrapSeed": {"min": 0, "max": 1000},
    },
}
...
def _check_bmr_value(v: float) -> float:
    if not (0 < v):
        raise PydanticCustomError("bmr", "BMR must be greater than 0; got {value}.", {"value": v})
    return v
