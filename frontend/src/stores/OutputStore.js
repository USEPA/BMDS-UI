...
class OutputStore {
  ...
  @computed get hasAnyError() {
    return (
      ...
      (this.selectedOutput && this.selectedOutput.error !== null)
    );
  }
  ...
  @computed get showContinuousDatasetFootnote() {
    const output = this.selectedOutput;
    return (
      this.getModelType === MODEL_CONTINUOUS && output && output.frequentist
    );
  }
  ...
  @computed get drModelSelected() {
    ...
    if (
      output &&
      output.frequentist &&
      _.isNumber(output.frequentist.selected.model_index)
    ) {
      const model =
        output.frequentist.models[output.frequentist.selected.model_index];
  ...
  @computed get selectedMultitumorModels() {
    ...
    return results.selected_model_indexes.map(
      (idx, i) => results.models[i][idx],
    );
  }
  ...
  @action.bound showModalDetailMultitumor(i, j) {
    ...
    this.modalDataset = isMa
      ? null
      : this.rootStore.dataStore.datasets[datasetIndexes[i]];
  ...
  @computed get drIndividualPlotLayout() {
    // a single model, shown in the modal
    const selectedModel = this.showSelectedModelInModalPlot
  ...
  @action.bound saveSelectedIndexNotes(value) {
    this.selectedOutput.frequentist.selected.notes =
  ...
  @action.bound updateUserPlotSettings(e) {
    ...
    if (_.has(e, "xaxis.range[0]") && _.has(e, "xaxis.range[1]")) {
      this.userPlotSettings["xaxis"] = [
        e["xaxis.range[0]"],
        e["xaxis.range[1]"],
      ];
    }
    if (_.has(e, "yaxis.range[0]") && _.has(e, "yaxis.range[1]")) {
      this.userPlotSettings["yaxis"] = [
        e["yaxis.range[0]"],
        e["yaxis.range[1]"],
      ];
    }
