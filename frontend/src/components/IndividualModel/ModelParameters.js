...
const seFootnote = (
    <p className="text-sm">
        ...
        estimates may not be reliable as a basis for confidence intervals or tests when one or more
        parameters are on bounds.
    </p>
);
const getData = model => {
    const parameters = model.results.parameters;
    const indexes = _.range(parameters.names.length);
    const anyBounded = _.sum(parameters.bounded) > 0;
...
const getNestedData = model => {
    const names = model.results.parameter_names;
    const values = model.results.parameters;
    const bounded = model.results.bounded;
    const se = model.results.std_err;
    const anyBounded = _.some(bounded);
    return {
        tblClasses: "table table-sm text-right col-l-1",
        headers: ["Variable", "Estimate", "Standard Error"],
        subheader: "Model Parameters",
...
class ModelParameters extends Component {
    render() {
        ...
        const fn = isNestedDichotomous ? getNestedData : getData;
