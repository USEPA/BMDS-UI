...
class MCMCStore {
  ...
  @action.bound resetDistTypeToDefault() {
    if (this.getModelType !== MODEL_CONTINUOUS) return;
    const def = constant.options[MODEL_CONTINUOUS].dist_type;
  ...
  @action.bound saveOptions(name, value, id) {
    ...
    if (name === constant.BMR_TYPE && this.getModelType === MODEL_CONTINUOUS) {
      ...
      this.optionsList[id][constant.BMR_VALUE] =
        constant.bmrForBmrTypeContinuous[value];
    }
  }
